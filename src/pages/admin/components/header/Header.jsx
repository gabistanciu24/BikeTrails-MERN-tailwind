...
const Header = () => {
  ...
  const { mutate: mutateCreatePost, isLoading: isLoadingCreatePost } =
    useMutation({
      ...
      onSuccess: (data) => {
        ...
        toast.success("Track created, edit that now!");
  ...
  return (
    <header className="flex h-fit w-full items-center justify-between p-4 lg:h-full lg:max-w-[18.75rem] lg:flex-col lg:items-start lg:justify-start lg:p-0">
      {/* logo */}
      <Link to="/">
        <img src={images.HeroLogo} alt="logo" className="w-16 h-16 lg:hidden" />
      </Link>
      {/* menu burger icon */}
      ...
      {isMenuActive && (
        <div className="fixed inset-0 lg:static lg:h-full lg:w-full">
          ...
          <div className="fixed top-0 bottom-0 left-0 z-50 w-3/4 overflow-y-auto bg-white p-4 lg:static lg:h-full lg:w-full lg:p-6">
            <Link to="/">
              <img src={images.HeroLogo} alt="logo" className="w-16" />
            </Link>
            <h4 className="mt-10 font-bold text-[#C7C7C7]">MENIU PRINCIPAL</h4>
            {/* menu items */}
            <div className="mt-6 flex flex-col gap-y-[0.563rem]">
              ...
              <NavItemCollapse
                ...
                setActiveNavName={setActiveNavName}
              >
                <Link to="/admin/posts/manage">Manage all trails</Link>
                <button
